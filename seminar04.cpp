...
public:
	...
	Magazin(const Magazin& m) :anDeschidere(m.anDeschidere) { //constructor de copiere
		this->oras = m.oras;
	...
	void setNrAngajati(int nrAngati, float* salarii) {
		if (nrAngajati > 0) {
			this->nrAngajati = nrAngajati;
	...
	static float calculeazaSuprafataTotala(Magazin* vector, int nrMagazin) {
		float suma = 0; {
			for (int i = 0; i < nrMagazin; i++) {
	...
	int main() {
		...
		Magazin m3(m2);
		Magazin m4 = m2; //similar cu ce e mai sus
