...
public:
	...
	friend ostream& operator << (ostream& o, const ComplexComercial& cc) {
		...
		o << *cc.cofetarie << endl; // se apeleaza op << din cofetarie
	...
	ComplexComercial operator--() {
		if (this->nrMasiniParcate > 0) {
			...
			delete[]this->masiniParcate;
			this->masiniParcate = aux;  // se apeleaza op = pt pointeri, nimic din clasa masina
		}
	...
	ComplexComercial operator--(int) {
		ComplexComercial cc = *this; //se apeleaza constructorul de copiere din clasa ComplexComercial
		--(*this);
		return cc;
	}
